        ...
        action.setCallback(this, function (response) {
            ...
            if (response.getState() === "SUCCESS") {
                if (!$A.util.isEmpty(result)) {
                    ...
                    console.log('###### approvalList2: ', result);
    ...
    handleSelectedRows: function (component, event, actionTask) {
                ...
                for (var ind2 = 0; ind2 < approvalList[index].data.length; ind2++) {
                    if (approvalList[index].data[ind2].isChecked)
                        selectedProcesses.push(
                            approvalList[index].data[ind2].processInstanceId
                        );
                }
            }
        }
        if (!$A.util.isEmpty(selectedProcesses) && selectedProcesses.length > 0) {
            ...
            self.showToast(
                component,
                "Info",
                "Please select Record(s) to Approve/Reject."
            );
        }
    },

    updateApprovalHelper: function (
        component,
        selectedProcesses,
        comment,
        actionTask
